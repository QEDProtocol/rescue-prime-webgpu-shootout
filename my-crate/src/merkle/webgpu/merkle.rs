...
pub async fn web_gpu_generate_merkle_tree(helper: &WebGpuHelper, leaves: &[[Felt; 4]]) -> Option<Vec<[Felt; 4]>> {
  ...
  */
  ...
  let res = receiver.recv_async().await.unwrap().unwrap();
